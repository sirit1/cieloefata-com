...
export const tratados: Tratado[] = [
  {
    ...
    cuerpo: [
      ...
      "Cada eslabón depende del anterior y prepara el posterior. Invertir el orden no es adaptar, sino eiségesis. Nadie llega a Decisión sin haber pasado por Ver. Nadie invoca Doctrina para ahorrarse Revelación. Nadie usa Argumento como primer movimiento. El que empieza por A para ganar una polémica no está haciendo Éfata, sino teatro. El milagro, no obstante, sigue siendo de Marcos 7: Éfata, sé abierto. El método es el cauce, no el milagro. Si el acrónimo se sienta en el trono, se ha vuelto a la sordera con más libros.",
  ...
  },
  {
    ...
    blurb:
      "Oír y no hacer no es un retraso inocente, sino engaño de sí. Santiago pone el espejo delante, y el que se mira y se va olvida el rostro. Este tratado es la compuerta: lo oído se prueba hasta que quede un acto, no un archivo.",
    pack: true,
    cuerpo: [
  ...
  },
  {
    ...
    blurb:
      "«Buscad primeramente el reino» no es un lema de carrera ni una técnica para que lo demás se añada como premio. Jesús habla a oídos ansiosos, en el Sermón del Monte, y el reino que se busca no es un proyecto personal, sino el Padre que ya sabe.",
    pack: true,
    cuerpo: [
  ...
  },
  {
    ...
    cuerpo: [
      ...
      "Por tanto, si tu «muerte y vida» no cabe en un proverbio que también manda callar, no es este texto. El tratado quita el decreto y devuelve el temor. Callar para ganar, en esta casa, no es huir, sino no encender el bosque por una chispa, y hablar cuando el pasaje manda hablar. El que usa 18:21 para no oír a Santiago ha fabricado otra lengua: la del mago, no la del testigo.",
  ...
  },
  {
    ...
    blurb:
      "«Si Dios es por nosotros, ¿quién contra nosotros?» no es un grito de invencibilidad ni un escudo de campaña. Pablo pregunta después de la cadena de 8:28–30, y la respuesta no es que nadie se oponga, sino que nadie puede condenar al que el Hijo compró.",
    pack: true,
    cuerpo: [
